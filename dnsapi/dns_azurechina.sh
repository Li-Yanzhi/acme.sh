...
dns_azurechina_add() {
  ...
  AZUREDNS_RESOURCEGROUP="${AZUREDNS_RESOURCEGROUP:-$(_readaccountconf_mutable AZUREDNS_RESOURCEGROUP)}"
  ...
  if [ -z "$AZUREDNS_RESOURCEGROUP" ]; then
    AZUREDNS_SUBSCRIPTIONID=""
    AZUREDNS_TENANTID=""
    AZUREDNS_APPID=""
    AZUREDNS_CLIENTSECRET=""
    AZUREDNS_RESOURCEGROUP=""
    _err "You didn't specify the Azure Resource Group"
    return 1
  fi
  ...
  _saveaccountconf_mutable AZUREDNS_RESOURCEGROUP "$AZUREDNS_RESOURCEGROUP"

  accesstoken=$(_azurechina_getaccess_token "$AZUREDNS_TENANTID" "$AZUREDNS_APPID" "$AZUREDNS_CLIENTSECRET")

  if ! _get_root "$fulldomain" "$AZUREDNS_SUBSCRIPTIONID" "$accesstoken" "$AZUREDNS_RESOURCEGROUP"; then
    _err "invalid domain"
    return 1
  fi
  ...
  acmeRecordURI="https://management.chinacloudapi.cn$(printf '%s' "$_domain_id" | sed 's/\\//g')/TXT/$_sub_domain?api-version=2018-05-01"
  _debug "$acmeRecordURI"
  # Get existing TXT record
  _azurechina_rest GET "$acmeRecordURI" "" "$accesstoken"
  ...
  _azurechina_rest PUT "$acmeRecordURI" "$body" "$accesstoken"
...
dns_azurechina_rm() {
  ...
  if [ "$_code" = "200" ]; then
    ...
    if [ -z "$values" ]; then
      ...
      _azurechina_rest DELETE "$acmeRecordURI" "" "$accesstoken"
...
_azurechina_getaccess_token() {
  ...
  body="resource=$(printf "%s" 'https://management.core.chinacloudapi.cn/' | _url_encode)&client_id=$(printf "%s" "$clientID" | _url_encode)&client_secret=$(printf "%s" "$clientSecret" | _url_encode)&grant_type=client_credentials"
  _secure_debug2 "data $body"
  response="$(_post "$body" "https://login.chinacloudapi.cn/$tenantID/oauth2/token" "" "POST")"
  _ret="$?"
  _secure_debug2 "response $response"
  response="$(echo "$response" | _normalizeJson)"
...
_get_root() {
  ...
  ##
  _azurechina_rest GET "https://management.chinacloudapi.cn/subscriptions/$subscriptionId/resourceGroups/$resourceGroup/providers/Microsoft.Network/dnszones?\$top=500&api-version=2018-05-01" "" "$accesstoken"
